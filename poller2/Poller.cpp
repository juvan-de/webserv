...
void			Poller::deleteSocket(int fd)
{
	...
	{
	case SERV:
		std::map<int, ServerSocket*>::iterator serv = _serv_socks.find(fd);
		delete serv->second;
		_serv_socks.erase(serv);
		break;
	case CLI:
		std::map<int, ClientSocket*>::iterator cli = _client_socks.find(fd);
		delete cli->second;
		_client_socks.erase(cli);
		break;
	case CGI:
		std::map<int, CgiSocket*>::iterator cgi = _cgi_socks.find(fd);
		delete cgi->second;
		_cgi_socks.erase(cgi);
		break;
	}
}
...
void			Poller::executePoll(std::map<std::pair<int, std::string>, Server*> table)
{
	...
	try
	{
		for (std::vector<pollfd>::const_iterator it = _pollfds.begin(); it != _pollfds.end(); it++)
		{
			...
			case CGI:
				getActiveFd(cgi, *it);
				// handlePoll(_cgi_socks.find(_pollfds[i].fd)->second, _pollfds[i].revents);
				break;
			}
		}
		handleCgi(cgi);
